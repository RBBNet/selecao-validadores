# language: pt
Funcionalidade: Atualização do código on-chain de seleção de validadores no ingress
  ...
  Cenário: Governança atualiza o contrato de seleção com sucesso no ingress
    ...
    E o endereço do novo contrato não é zero (0x0)
    ...
    Quando a função de atualização do código de seleção de validadores é executada no ingress
    ...
    E um evento registrando o contrato anterior e o contrato novo deve ser emitido
