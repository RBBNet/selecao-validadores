...
|-------|-----------|-------------|
| **Governança** | Modificar parâmetros, alterar contratos de lógica, adicionar/remover elegíveis, adicionar/remover operacionais (via endereço), atualizar contratos de permissionamento (governança, regras de contas e de nós). | Modificador `onlyGovernance` que valida via `AdminProxy.isAuthorized(msg.sender)`. |
