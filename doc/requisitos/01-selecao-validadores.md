...
2. O sistema deve validar:
   - Se os endereços dos contratos de permissionamento da RBB são válidos e implementam suas respectivas interfaces através de chamadas de teste:
     - `AdminProxy`: `isAuthorized(address(0))`
     - `AccountRules`: `transactionAllowed(address(0), address(0), 0, 0, 0, "")`
     - `NodeRules`: `connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)`
...
5.2. O sistema deve exigir a informação dos endereços dos contratos `AdminProxy`, `AccountRulesV2` e `NodeRulesV2`, validando que todos foram informados e implementam suas respectivas interfaces (através de chamadas de teste `isAuthorized`, `transactionAllowed` e `connectionAllowed`) antes da conclusão da implantação.  
...
* **Especificação Técnica:**
  * [Modelo de Dados e Interfaces](../especificacao-tecnica/01-modelo-contratos.md) — Storage, interfaces, eventos e erros do contrato `ValidatorSelection`.
  * [Função `constructor` do `ValidatorSelection`](../especificacao-tecnica/02-especificacao-funcoes.md#321-constructor) — Especificação detalhada da função de implantação.
* **Arquitetura:**
  * [DA-03: Desacoplamento com Permissionamento](../arquitetura/01-visao-geral.md#da-03) — Justificativa para não validar estado de permissionamento no cadastro.
  * [DA-04: Algoritmo com Dois Parâmetros](../arquitetura/01-visao-geral.md#da-04) — Justificativa para `blocksBetweenSelection` e `blocksWithoutProposeThreshold`.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Justificativa para o conjunto de validadores protegidos.
  * [DA-09: Suporte ao Padrão ERC-165](../arquitetura/01-visao-geral.md#da-09) — Suporte a introspecção de interfaces.
...
* **Arquitetura:**
  * [DA-01: Padrão Ingress/Facade sem Proxy de Storage](../arquitetura/01-visao-geral.md#da-01) — Justificativa para o uso de fachada fixa no Besu e configuração no arquivo gênesis.
...
* **Especificação Técnica:**
  * [Função `getValidators` do `ValidatorSelection`](../especificacao-tecnica/02-especificacao-funcoes.md#3214-getvalidators) — Especificação detalhada da consulta de validadores operacionais.
...
* **Especificação Técnica:**
  * [Função `getEligibleValidators`](../especificacao-tecnica/02-especificacao-funcoes.md#3213-geteligiblevalidators) — Especificação detalhada da consulta de validadores elegíveis.
...
* **Especificação Técnica:**
  * [Função `setOperationMode`](../especificacao-tecnica/02-especificacao-funcoes.md#322-setoperationmode) — Especificação detalhada com regras de transição para o modo automático.
  * [Enum `OperationMode`](../especificacao-tecnica/01-modelo-contratos.md#23-interface-comum-eventos-e-erros) — Definição do enum `Manual`/`Automatic`.
* **Arquitetura:**
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Proteção de validadores ao transicionar para modo Automatic.
...
* **Especificação Técnica:**
  * [Função `setSelectionParameters`](../especificacao-tecnica/02-especificacao-funcoes.md#3212-setselectionparameters) — Especificação detalhada da configuração de parâmetros.
* **Arquitetura:**
  * [DA-04: Algoritmo com Dois Parâmetros](../arquitetura/01-visao-geral.md#da-04) — Justificativa para os dois parâmetros de monitoração.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Reset do conjunto de protegidos ao reconfigurar parâmetros.
...
4.8. Para o conjunto de validadores pré-selecionados que não pertencem ao conjunto de protegidos, a exclusão em lote dependerá da quantidade mínima de segurança da rede:
* Remoção Autorizada: Se ao menos 4 validadores operacionais permanecerem ativos na rede após a exclusão de **todo o conjunto** de nós avaliados, o sistema deve, para cada um deles:
...
* Remoção Vetada: Se a exclusão do conjunto total deixar a rede com menos de 4 validadores operacionais, **nenhum** dos nós inoperantes deve ser removido, sendo todos mantidos como validadores operacionais para proteger o consenso.
...
5.6. Caso ocorra a exclusão de nós operacionais inoperantes, o sistema deve avaliar a remoção do conjunto completo de nós pré-selecionados. Se a remoção de todos eles resultar em menos de 4 nós operacionais ativos, a exclusão de todo o conjunto é vetada por segurança. Caso contrário, todos os nós pré-selecionados são removidos e para cada exclusão efetivada, um evento específico identificando o validador removido deve ser disparado.  
5.7. Ao fechar a avaliação do ciclo, o sistema deve executar as ações definidas em 4.9.

##### Referências Técnicas

* **Especificação Técnica:**
  * [Função `executeMonitoring`](../especificacao-tecnica/02-especificacao-funcoes.md#323-executemonitoring) — Algoritmo detalhado com as três fases: identificação de inativos, filtro/remoção e reset de ciclo.
* **Arquitetura:**
  * [DA-04: Algoritmo com Dois Parâmetros](../arquitetura/01-visao-geral.md#da-04) — Justificativa para os parâmetros de monitoração.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Imunidade temporária de validadores protegidos.
  * [DA-06: Remoção Agrupada de Inoperantes](../arquitetura/01-visao-geral.md#da-06) — Remoção de todos os inoperantes de uma vez.
  * [DA-08: Sem Restrição de Acesso na Monitoração](../arquitetura/01-visao-geral.md#da-08) — Justificativa para acesso livre à função.
...
* **Especificação Técnica:**
  * [Função `addOperationalValidator` (via Enode)](../especificacao-tecnica/02-especificacao-funcoes.md#324-addoperationalvalidator-via-enode) — Especificação via chave pública.
  * [Função `addOperationalValidator` (via Endereço)](../especificacao-tecnica/02-especificacao-funcoes.md#325-addoperationalvalidator-via-endereço) — Especificação via endereço.
  * [Regras Internas: Verificação de Administrador e Vínculo Organizacional](../especificacao-tecnica/02-especificacao-funcoes.md#3216-regras-internas-de-validação-e-processamento) — Validação de papéis e escopo organizacional.
* **Arquitetura:**
  * [DA-03: Desacoplamento com Permissionamento](../arquitetura/01-visao-geral.md#da-03) — Justificativa para não validar estado de permissionamento no cadastro.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Proteção de validadores recém-adicionados.
...
* **Especificação Técnica:**
  * [Função `removeOperationalValidator` (via Enode)](../especificacao-tecnica/02-especificacao-funcoes.md#326-removeoperationalvalidator-via-enode) — Especificação via chave pública.
  * [Função `removeOperationalValidator` (via Endereço)](../especificacao-tecnica/02-especificacao-funcoes.md#327-removeoperationalvalidator-via-endereço) — Especificação via endereço.
  * [Regras Internas: Verificação de Administrador e Vínculo Organizacional](../especificacao-tecnica/02-especificacao-funcoes.md#3216-regras-internas-de-validação-e-processamento) — Validação de papéis e escopo organizacional.
* **Arquitetura:**
  * [DA-03: Desacoplamento com Permissionamento](../arquitetura/01-visao-geral.md#da-03) — Justificativa para não validar estado de permissionamento.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Remoção do conjunto de protegidos.
...
* **Especificação Técnica:**
  * [Função `addEligibleValidator` (via Endereço)](../especificacao-tecnica/02-especificacao-funcoes.md#328-addeligiblevalidator-via-endereço) — Especificação via endereço com ajuste automático de parâmetros.
  * [Função `addEligibleValidator` (via Enode)](../especificacao-tecnica/02-especificacao-funcoes.md#329-addeligiblevalidator-via-enode) — Especificação via chave pública.
* **Arquitetura:**
  * [DA-03: Desacoplamento com Permissionamento](../arquitetura/01-visao-geral.md#da-03) — Justificativa para não validar estado de permissionamento.
  * [DA-05: Proteção via Conjunto Temporário](../arquitetura/01-visao-geral.md#da-05) — Proteção de validadores recém-adicionados como operacionais.
...
* **Especificação Técnica:**
  * [Função `removeEligibleValidator` (via Endereço)](../especificacao-tecnica/02-especificacao-funcoes.md#3210-removeeligiblevalidator-via-endereço) — Especificação via endereço.
  * [Função `removeEligibleValidator` (via Enode)](../especificacao-tecnica/02-especificacao-funcoes.md#3211-removeeligiblevalidator-via-enode) — Especificação via chave pública.
* **Arquitetura:**
  * [DA-03: Desacoplamento com Permissionamento](../arquitetura/01-visao-geral.md#da-03) — Justificativa para não validar estado de permissionamento.
...
* **Especificação Técnica:**
  * [Função `updateValidatorSelectionContract` do `Ingress`](../especificacao-tecnica/02-especificacao-funcoes.md#313-updatevalidatorselectioncontract) — Especificação detalhada da atualização do contrato de lógica no Ingress.
* **Arquitetura:**
  * [DA-01: Padrão Ingress/Facade sem Proxy de Storage](../arquitetura/01-visao-geral.md#da-01) — Atualização via reponteiramento no Ingress.
  * [DA-02: Validação try/catch para Interfaces Legadas](../arquitetura/01-visao-geral.md#da-02) — Validação dinâmica de `getValidators()`.
...
* **Especificação Técnica:**
  * [Função `supportsInterface` do `ValidatorSelection`](../especificacao-tecnica/02-especificacao-funcoes.md#3215-supportsinterface) — ERC-165 no contrato de Lógica.
  * [Função `supportsInterface` do `Ingress`](../especificacao-tecnica/02-especificacao-funcoes.md#317-supportsinterface) — ERC-165 no contrato Ingress.
  * [Interfaces Solidity](../especificacao-tecnica/01-modelo-contratos.md#23-interface-comum-eventos-e-erros) — Definição das interfaces `IValidatorSelection` e `IValidatorSelectionIngress`.
* **Arquitetura:**
  * [DA-09: Suporte ao Padrão ERC-165](../arquitetura/01-visao-geral.md#da-09) — Justificativa para implementação do ERC-165.
...
* **Especificação Técnica:**
  * [Função `updateAdminContract` do `ValidatorSelection`](../especificacao-tecnica/02-especificacao-funcoes.md#3217-updateadmincontract) — Especificação detalhada do reponteiramento do AdminProxy na Lógica.
...
2. O sistema deve validar:
   - Se o endereço informado não é nulo (`address(0)`).
   - Se o endereço informado é diferente do endereço ativo atualmente.
   - Se o novo contrato implementa de fato a interface `AccountRulesProxy` realizando uma chamada de teste (`transactionAllowed(address(0), address(0), 0, 0, 0, "")`).
...
4.2. O novo contrato de regras de contas deve responder com sucesso à chamada `transactionAllowed(address(0), address(0), 0, 0, 0, "")`.
...
5.1. A execução desta atualização deve ser exclusiva da governança autorizada.  
5.2. O sistema deve exigir e validar que o endereço fornecido não seja nulo e que seja diferente do endereço do contrato atualmente ativo.  
5.3. O sistema deve validar que o novo endereço implementa a interface correta por meio de uma chamada de teste `transactionAllowed(address(0), address(0), 0, 0, 0, "")`. Caso falhe ou reverta, a transação deve ser revertida com erro.  
...
* **Especificação Técnica:**
  * [Função `updateAccountsContract`](../especificacao-tecnica/02-especificacao-funcoes.md#3218-updateaccountscontract) — Especificação detalhada do reponteiramento do AccountRulesV2 na Lógica.
...
2. O sistema deve validar:
   - Se o endereço informado não é nulo (`address(0)`).
   - Se o endereço informado é diferente do endereço ativo atualmente.
   - Se o novo contrato implementa de fato a interface `NodeRulesProxy` realizando uma chamada de teste (`connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)`).
...
4.2. O novo contrato de regras de nós deve responder com sucesso à chamada `connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)`.
...
5.1. A execução desta atualização deve ser exclusiva da governança autorizada.  
5.2. O sistema deve exigir e validar que o endereço fornecido não seja nulo e que seja diferente do endereço do contrato atualmente ativo.  
5.3. O sistema deve validar que o novo endereço implementa a interface correta por meio de uma chamada de teste `connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)`. Caso falhe ou reverta, a transação deve ser revertida com erro.  
...
* **Especificação Técnica:**
  * [Função `updateNodesContract`](../especificacao-tecnica/02-especificacao-funcoes.md#3219-updatenodescontract) — Especificação detalhada do reponteiramento do NodeRulesV2 na Lógica.
