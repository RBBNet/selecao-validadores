...
contract ValidatorSelectionIngressTest is ValidatorSelectionBaseTest {
    ...
    function _assertFallbackToProposer() internal view {
    ...
    function test_constructor_AcceptsAdminContractWithZeroAddressAuthorized() public {
        AdminMock newAdminMock = new AdminMock();
        newAdminMock.addAdmin(address(0));

        ValidatorSelectionIngress newIngress =
            new ValidatorSelectionIngress(address(newAdminMock), address(selectionMock));

        assertEq(address(newIngress.admins()), address(newAdminMock));
    }
    ...
    function test_getValidators_DelegatesToSelectionContract() public view {
        address[] memory validators = ingress.getValidators();
    ...
    function test_getValidators_AnyAccountCanQuery() public {
        ...
        ingress.getValidators(); // should not revert
    }
    ...
    function test_updateValidatorSelectionContract_RevertsIfNoCode() public {
        address noCode = address(0x9999);
        vm.prank(governance);
        bytes memory expectedError = abi.encodeWithSelector(InvalidValidatorSelectionContract.selector, noCode);
        vm.expectRevert(expectedError);
        ingress.updateValidatorSelectionContract(noCode);
    }
