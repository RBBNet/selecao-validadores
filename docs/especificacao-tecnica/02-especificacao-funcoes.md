...
*   **Validações:**
    ...
    *   `_adminProxy` deve responder com sucesso à chamada `isAuthorized(address(0))` (caso contrário, reverte com `InvalidAdminContract(_adminProxy)`).
    *   `_accountsContract` deve responder com sucesso à chamada `transactionAllowed(address(0), address(0), 0, 0, 0, "")` (caso contrário, reverte com `InvalidAccountsContract(_accountsContract)`).
    *   `_nodesContract` deve responder com sucesso à chamada `connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)` (caso contrário, reverte com `InvalidNodesContract(_nodesContract)`).
...
*   **Algoritmo de Execução:**
    ...
    5. Se `block.number >= nextSelectionBlock` (Ciclo de Seleção Automática) *(Nota: a condição utiliza `>=` ao invés de `==` por resiliência — garante que o ciclo de seleção seja executado mesmo que nenhuma transação de monitoração tenha sido processada exatamente no bloco alvo, cobrindo cenários de blocos sem monitoração.)*:
        ...
        *   **Fase 2: Filtro e Remoção.** 
            *   Determina-se um conjunto temporário de validadores pré-selecionados para remoção, filtrando e ignorando aqueles que estejam protegidos (`protectedValidators.contains(V)`).
            *   Se a exclusão conjunta de todos os validadores pré-selecionados preservar o limite de segurança da rede (`operationalValidators.length - quantidade_a_remover >= 4`):
                *   Remove cada `V` pré-selecionado de `operationalValidators` e emite `OperationalValidatorRemoved(V)`.
            *   Se a exclusão conjunta violar o mínimo de 4 operacionais, a remoção em lote é abortada e **nenhum** validador é removido (todos os nós inoperantes são mantidos para proteger o consenso).
...
*   **Validações:**
    ...
    *   `_newAccountsContract` deve responder com sucesso à chamada externa `transactionAllowed(address(0), address(0), 0, 0, 0, "")` realizada dentro de um bloco `try/catch`. Caso falhe ou reverta, a execução é interrompida revertendo com `InvalidAccountsContract(_newAccountsContract)`.
*   **Efeitos:**
...
*   **Validações:**
    ...
    *   `_newNodesContract` deve responder com sucesso à chamada externa `connectionAllowed(bytes32(0), bytes32(0), bytes16(0), 0, bytes32(0), bytes32(0), bytes16(0), 0)` realizada dentro de um bloco `try/catch`. Caso falhe ou reverta, a execução é interrompida revertendo com `InvalidNodesContract(_newNodesContract)`.
*   **Efeitos:**
