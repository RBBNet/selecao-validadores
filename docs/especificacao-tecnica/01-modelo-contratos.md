...
Tanto o contrato `ValidatorSelection` quanto o `ValidatorSelectionIngress` devem implementar o padrão **ERC-165** (`supportsInterface`) para consulta e validação de interfaces.
...
| `InvalidAdminContract(address addr)` | `address addr` | Local (`ValidatorSelectionIngress` / `ValidatorSelection`) | Contrato admin proxy fornecido não implementou ou falhou na chamada de teste `isAuthorized(address(0))`. | [US1-1](../requisitos/01-selecao-validadores.md#us1-1), [US1-2](../requisitos/02-ingress-selecao-validadores.md#us1-2), [US5-5](../requisitos/02-ingress-selecao-validadores.md#us5-5), [US5-7](../requisitos/01-selecao-validadores.md#us5-7) |
| `InvalidAccountsContract(address addr)` | `address addr` | Local (`ValidatorSelection`) | Contrato de regras de contas fornecido não implementou ou falhou na chamada de teste `transactionAllowed(...)` da interface `AccountRulesProxy`. | [US1-1](../requisitos/01-selecao-validadores.md#us1-1), [US5-8](../requisitos/01-selecao-validadores.md#us5-8) |
| `InvalidNodesContract(address addr)` | `address addr` | Local (`ValidatorSelection`) | Contrato de regras de nós fornecido não implementou ou falhou na chamada de teste `connectionAllowed(...)` da interface `NodeRulesProxy`. | [US1-1](../requisitos/01-selecao-validadores.md#us1-1), [US5-9](../requisitos/01-selecao-validadores.md#us5-9) |
...
### 2.4.3 `AccountRulesProxy`
Responsável pela verificação da permissão de execução de transações na rede:
```solidity
...
### 2.4.5 `NodeRulesProxy`
Responsável pela verificação da permissão de conexão entre nós na rede:
```solidity
